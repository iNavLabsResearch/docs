...
Advanced behavior supports an auto-terminate flag in `agent_specific_config`: use `auto_terminate_call` (backend key). Dashboard/frontend may also refer to it as `autoTerminate`; both mean the same behavior (agent can invoke call cut/hangup automatically when needed).
