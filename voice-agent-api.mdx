...
The key is tied to a **user account** in Kupe (same as dashboard login). That user’s UUID is required on several routes (for example file upload form fields and outbound `user_id` query parameter). To read it programmatically, call **`GET /api/v1/me`** with the same key — the JSON field **`user_id`** is the UUID.
...
```json
{
  ...
  "webhook_url": "https://api.kupe.in/api/call/incoming_call",
  "config_source": "<same-label-as-in-dashboard-for-this-line>",
  "user_id": "<owner-user-uuid>"
}
```

- Send exactly one of **`agent_id`** or **`workflow_id`**.
- **`user_id`** must own the line — use **`GET /api/v1/me`** when integrating with your own API key.
- **`webhook_url`** and **`config_source`** must match what Kupe expects for that line (copy from the dashboard or your deployment runbook).
...
This route is **not** under `/api/v1` and is listed in auth exclusions: it does not require `x-api-key` today, but you should still protect it at the network edge because it accepts `user_id` as a query parameter.

| Query parameter | Required | Description |
|-----------------|----------|-------------|
| `phoneNumber` | Yes | Destination — use **E.164** with country code (e.g. **+91**… India, **+1**… US/Canada); normalized server-side |
| `user_id` | Yes | Owner user UUID |
...
```bash
curl -sS -X POST 'https://api.kupe.in/api/call/create_call?phoneNumber=%2B15551234567&user_id=<USER_UUID>&agent_id=<AGENT_UUID>'
```

India-style destination (URL-encoded `+`):

```bash
curl -sS -X POST 'https://api.kupe.in/api/call/create_call?phoneNumber=%2B919876543210&user_id=<USER_UUID>&agent_id=<AGENT_UUID>'
```
