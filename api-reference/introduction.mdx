...
Use the **left sidebar** (OpenAPI tag groups, including **Service**, **Current user**, **Incoming calls**, **Call logs**, **Billing and usage**, **Wallets**, **Earnings**, **Tools and MCP**, **Post-call analysis**, **API keys**, then **Agent Management**, **Providers**, **Noise Assets**, **Outbound calls**, **Knowledge base**, **Webhooks**) to open each operation. Every page includes **Try it**, request parameters, and response **schemas**. Tag names avoid `&` so Mintlify can render every group reliably.
...
- **GET** `/api/v1/noise-assets` — Background noise WAV catalog for **`agent_specific_config.background_noise_id`**.

### Outbound calls

- **POST** `/api/call/create_call` — Outbound PSTN (query params + optional JSON `variable_values` for `{{prompt}}` substitution). **Auth:** `x-api-key` (recommended) or `Authorization: Bearer` — that user is billed and their phone/agent mappings apply (**no** `user_id` query). Response includes **`request_id`**, **`call_session_id`**, optional **`telephony_leg_id`**; `telephony_provider` is always `pstn` in JSON.
...
| **timeouts / auto hangup / transfer** | `agent_specific_config` on **`POST /api/v1/agents`** or **`PUT /api/v1/agents/{agent_id}`** (`auto_terminate_call`, `session_timeout`, `inbound_transfer_routes`, `time_to_cut`, …). |
| **inbound noise reduction** | `agent.apply_noise_reduction` + `noise_cancellation_strength` in `agent_specific_config`. |
| **outbound background noise** | **`GET /api/v1/noise-assets`** → `background_noise_id` (+ optional `background_noise_volume`). |
| **extracted_variables** | Post-analysis structured output / HTTP tool (see **Webhooks**). |
| **context_params** | Optional JSON on `POST /api/call/create_call`: `{ "variable_values": { … } }` — replaces `{{key}}` in system prompt and first response at dial time. |
