...
- **POST** `/api/call/create_call` — Outbound PSTN (`phoneNumber`, optional `user_id`, optional `agent_id`, …). If `user_id` is omitted, backend resolves from `x-api-key` or Bearer auth. Response **`call_sid`** = your **`call_id`**.
...
Your key is bound to a **user** in Kupe. Some routes still need that user’s UUID directly (for example `user_id` on file upload); `create_call` can now infer `user_id` from auth when omitted.

### Public routes

- **GET** `/health` — no API key.
- **POST** `/api/call/create_call` — optional `Origin` allow-list applies. You may pass `user_id` directly, or authenticate with `x-api-key` / Bearer and omit it.
